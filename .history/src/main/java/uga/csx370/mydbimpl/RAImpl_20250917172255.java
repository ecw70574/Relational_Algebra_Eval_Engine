...
public class RAImpl implements RA {
    ...
    public Relation intersect(Relation rel1, Relation rel2) {
        ...
        for (int i = 0; i < rel1.getAttrs().size(); i++) {
            ...
            // Step 2: Create an empty relation with the same schema as rel1
        Relation result = new RelationBuilder()
            .attributeNames(rel1.getAttrs())   // use the same column names
            .attributeTypes(rel1.getTypes())   // use the same column types
            .build();                          // build the new empty relation
